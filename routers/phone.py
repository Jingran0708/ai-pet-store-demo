...
def _new_call() -> dict:
    return {
        ...
        "last_prompt": "",
        "noinput_count": 0,
        "awaiting_callback_number": False,
    }
...
<Response>
    <Gather input="{input_attr}" action="/voice/respond" method="POST" speechTimeout="3" timeout="8"
        ...
        <Say voice="Polly.Joanna-Neural">{safe_text}{extra}</Say>
    </Gather>
...
async def voice_respond(request: Request):
    ...
    if _wants_live_agent(user_text) and not call.get("awaiting_callback_number"):
        name = call["state"].get("name")
        name_part = f" {name}," if name else ","
        reply = (
            f"I'm sorry{name_part} our store agent is currently not available to speak on the phone. "
            "I'm trained to help you and your furry friend, so if you need to ask something about buying a pet, "
            "booking an appointment, or anything else, you can talk to me. If you still need to speak with an "
            "agent, you can leave your contact number and our store agent will get back to you shortly."
        )
        call["history"].append({"role": "assistant", "content": reply})
        call["awaiting_callback_number"] = True
        return _twiml(reply, call=call, allow_keypad=True)

    if call.get("awaiting_callback_number"):
        phone_match = re.search(r"(\+?1?[\s.-]?)?\(?\d{3}\)?[\s.-]?\d{3}[\s.-]?\d{4}", user_text)
        call["awaiting_callback_number"] = False
        if phone_match or digits:
            number_to_leave = phone_match.group(0) if phone_match else digits
            store_html = f"""
            <h2>Customer Requested a Callback</h2>
            <p><strong>Name:</strong> {call['state'].get('name') or 'Unknown'}</p>
            <p><strong>Callback number:</strong> {number_to_leave}</p>
            """
            send_email(STORE_EMAIL, "Customer Callback Request", store_html)
            reply = "Thank you, I've passed your number along and our team will reach out to you shortly. Is there anything else I can help you with in the meantime?"
        else:
            reply = "No problem — feel free to call back anytime if you'd like to leave a number. Is there anything else I can help you with?"
        call["history"].append({"role": "assistant", "content": reply})
        return _twiml(reply, call=call)
...
def _wants_live_agent(user_text: str) -> bool:
    """True if the caller is explicitly asking to speak with a human/live agent/real person."""
    lower = user_text.lower()
    phrases = [
        "speak with a live agent", "speak with an agent", "speak with a real person",
        "talk to a live agent", "talk to an agent", "talk to a real person",
        "talk with an agent", "talk with a real person", "talk with a live agent",
        "speak to a human", "talk to a human", "real person", "human agent",
        "speak with someone", "talk to someone else", "customer service representative",
    ]
...
def _do_booking(call: dict, caller_number: str) -> str:
    ...
    is_medical_concern = s.get("intent") == "aftersale" and _sounds_medical(reason)
    if is_medical_concern:
        confirmation = (
            f"Your appointment for your furry friend has been confirmed, and a text message will be sent "
            f"shortly. If there's an emergency with your pet before then, please reach out to a vet right away."
        )
    else:
        confirmation = (
            f"Great news, your appointment is booked! Your confirmation number is {appt_id}, "
            f"and a text message will be sent shortly."
        )

    return f"{confirmation} Is there anything else I can help you with?"


def _sounds_medical(reason: str) -> bool:
    """Heuristic check for whether a reason describes a pet health/behaviour concern (for tone purposes only)."""
    if not reason:
        return False
    keywords = [
        "sick", "ill", "vomit", "diarrhea", "not eating", "won't eat", "lethargic", "limping",
        "injury", "injured", "pain", "fever", "bleeding", "behaviour", "behavior", "anxious",
        "aggressive", "won't stop", "throwing up", "infection", "wound", "swelling", "allergic",
    ]
    lower = reason.lower()
